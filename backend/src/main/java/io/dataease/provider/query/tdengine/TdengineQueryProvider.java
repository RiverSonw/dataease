package io.dataease.provider.query.tdengine;
...
import io.dataease.plugins.common.constants.datasource.TDengineConstants;
...
public class TdengineQueryProvider extends QueryProvider {
        ...
        if (f.getDeExtractType() == 1) {
            if (f.getDeType() == 2 || f.getDeType() == 3) {
                ...
                if (f.getType().equalsIgnoreCase("YEAR")) {
                    fieldName = String.format(TDengineConstants.DATE_FORMAT, "CONCAT(" + originField + ",'-01-01')", TDengineConstants.DEFAULT_DATE_FORMAT);
                } else if (f.getType().equalsIgnoreCase("TIME")) {
                    fieldName = String.format(TDengineConstants.DATE_FORMAT, "CONCAT('1970-01-01', " + originField + ")", TDengineConstants.DEFAULT_DATE_FORMAT);
                } else {
                    fieldName = String.format(TDengineConstants.DATE_FORMAT, originField, TDengineConstants.DEFAULT_DATE_FORMAT);
                }
            }
            ...
            if (f.getDeType() == 2) {
                ...
                fieldName = StringUtils.isEmpty(f.getDateFormat()) ? String.format(TDengineConstants.STR_TO_DATE, originField, TDengineConstants.DEFAULT_DATE_FORMAT) :
                        String.format(TDengineConstants.DATE_FORMAT, String.format(TDengineConstants.STR_TO_DATE, originField, f.getDateFormat()), TDengineConstants.DEFAULT_DATE_FORMAT);
            } else {
    ...
    public String createQuerySQL(String table, List<DatasetTableField> fields, boolean isGroup,
                                 Datasource ds, List<ChartFieldCustomFilterDTO> fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<DeSortField> sortFields) {
    ...
    public String getSQL(String table, List<ChartViewFieldDTO> xAxis, List<ChartViewFieldDTO> yAxis, List<ChartFieldCustomFilterDTO> fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<ChartExtFilterRequest> extFilterRequestList, Datasource ds, ChartViewWithBLOBs view) {
        ...
        if (CollectionUtils.isNotEmpty(xAxis)) {
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                    ...
                    originField = String.format(TDengineConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                } else {
                    originField = String.format(TDengineConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                }
                String fieldAlias = String.format(SQLConstants.FIELD_ALIAS_X_PREFIX, i);
        ...
        if (CollectionUtils.isNotEmpty(yAxis)) {
            for (int i = 0; i < yAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(y.getExtField()) && y.getExtField() == 2) {
                    ...
                    originField = String.format(TDengineConstants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
                } else {
                    originField = String.format(TDengineConstants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
                }
        ...
        SQLObj tableSQL = SQLObj.builder()
                .tableName(String.format(TDengineConstants.BRACKETS, sql))
            ...
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                    ...
                    if (x.getDeType() == 2 || x.getDeType() == 3) {
                        originField = String.format(TDengineConstants.CAST, String.format(TDengineConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName()), TDengineConstants.DEFAULT_FLOAT_FORMAT);
                    } else {
                        originField = String.format(TDengineConstants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                    }
                }
    ...
    public String createRawQuerySQL(String table, List<DatasetTableField> fields, Datasource ds) {
        String[] array = fields.stream().map(f -> {
            ...
            if (f.getDeExtractType() == 4) { // 处理 tinyint
                ...
                stringBuilder.append("").append(String.format(TDengineConstants.DATE_FORMAT, "CONCAT(" + f.getOriginName() + ",'-01-01')", TDengineConstants.DEFAULT_DATE_FORMAT)).append(" AS ").append(f.getDataeaseName());
            } else {
                stringBuilder.append("`").append(f.getOriginName()).append("` AS ").append(f.getDataeaseName());
            }
            return stringBuilder.toString();
        }).toArray(String[]::new);
        table = table.trim().startsWith("(") ? table : String.format(TDengineConstants.KEYWORD_TABLE, table);
        return MessageFormat.format("SELECT {0} FROM {1}  LIMIT DE_OFFSET, DE_PAGE_SIZE ", StringUtils.join(array, ","), table);
    }

    public String getTotalCount(boolean isTable, String sql, Datasource ds) {
        if (isTable) {
            return "SELECT COUNT(*) from " + String.format(TDengineConstants.KEYWORD_TABLE, sql);
        } else {
    ...
    public String transTreeItem(SQLObj tableObj, DatasetRowPermissionsTreeItem item) {
        ...
        if (field.getDeType() == 1) {
            if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5) {
                whereName = String.format(TDengineConstants.STR_TO_DATE, originName, StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : TDengineConstants.DEFAULT_DATE_FORMAT);
            }
        ...
        if (StringUtils.equalsIgnoreCase(item.getFilterType(), "enum")) {
            ...
            String whereTerm = transTdengineFilterTerm(item.getTerm());
            String whereValue = "";
     ...
     * @return
     */
    public String transTdengineFilterTerm(String term) {
        switch (term) {
            case "eq":
                return " = ";
            case "not_eq":
        ...
        for (ChartFieldCustomFilterDTO request : requestList) {
            ...
            if (StringUtils.equalsIgnoreCase(request.getFilterType(), "enum")) {
                ...
                for (ChartCustomFilterItemDTO filterItemDTO : filter) {
                    ...
                    String whereTerm = transTdengineFilterTerm(filterItemDTO.getTerm());
                    String whereValue = "";
    ...
    public String transExtFilterList(SQLObj tableObj, List<ChartExtFilterRequest> requestList) {
        ...
        for (ChartExtFilterRequest request : requestList) {
            ...
            for (DatasetTableField field : fieldList) {
                ...
                if (field.getDeType() == 1) {
                    ...
                    if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5 || field.getDeExtractType() == 1) {
                        String date = String.format(TDengineConstants.STR_TO_DATE, originName, StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : TDengineConstants.DEFAULT_DATE_FORMAT);
                        if(request.getOperator().equals("between")){
                            ...
                            if (StringUtils.equalsIgnoreCase(request.getDateStyle(), "y_Q")) {
                                whereName = String.format(format,
                                        String.format(TDengineConstants.DATE_FORMAT, originName, "%Y"),
                                        String.format(TDengineConstants.QUARTER, String.format(TDengineConstants.DATE_FORMAT, originName, TDengineConstants.DEFAULT_DATE_FORMAT)));
                            } else {
                                whereName = String.format(TDengineConstants.DATE_FORMAT, date, format);
                            }
                        }
                    ...
                    if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                        if(request.getOperator().equals("between")){
                            ...
                            if (StringUtils.equalsIgnoreCase(request.getDateStyle(),"y_Q")){
                                whereName = String.format(format,
                                        String.format(TDengineConstants.DATE_FORMAT, cast, "%Y"),
                                        String.format(TDengineConstants.QUARTER, String.format(TDengineConstants.DATE_FORMAT, field, TDengineConstants.DEFAULT_DATE_FORMAT)));
                            } else {
                                whereName = String.format(TDengineConstants.FROM_UNIXTIME, cast, format);
                            }
                        }
                    }
                } else if (field.getDeType() == 2 || field.getDeType() == 3) {
            ...
            String whereTerm = transTdengineFilterTerm(request.getOperator());
            String whereValue = "";
            ...
            if (StringUtils.containsIgnoreCase(request.getOperator(), "in")) {
                ...
                if (request.getDatasetTableField().getDeType() == 1) {
                    if (request.getDatasetTableField().getDeExtractType() == 2
                        ...
                        whereValue = String.format(TDengineConstants.WHERE_BETWEEN, value.get(0), value.get(1));
                    } else {
                        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                        String startTime = simpleDateFormat.format(new Date(Long.parseLong(value.get(0))));
                        String endTime = simpleDateFormat.format(new Date(Long.parseLong(value.get(1))));
                        whereValue = String.format(TDengineConstants.WHERE_BETWEEN, startTime, endTime);
                    }
                } else {
                    whereValue = String.format(TDengineConstants.WHERE_BETWEEN, value.get(0), value.get(1));
                }
            } else {
                whereValue = String.format(TDengineConstants.WHERE_VALUE_VALUE, value.get(0));
            }
            list.add(SQLObj.builder()
                    .whereField(whereName)
                    .whereTermAndValue(whereTerm + whereValue)
        ...
        if (x.getDeExtractType() == 1) {
                ...
                if (x.getType().equalsIgnoreCase("YEAR")) {
                    if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                        fieldName = String.format(format,
                                String.format(TDengineConstants.DATE_FORMAT, "CONCAT(" + originField + ",'-01-01')", "%Y"),
                                String.format(TDengineConstants.QUARTER, "CONCAT(" + originField + ",'-01-01')"));
                    } else {
                        fieldName = String.format(TDengineConstants.DATE_FORMAT, "CONCAT(" + originField + ",'-01-01')", format);
                    }
                    ...
                    if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                        fieldName = String.format(format,
                                String.format(TDengineConstants.DATE_FORMAT, originField, "%Y"),
                                String.format(TDengineConstants.QUARTER, originField));
                    } else {
                        fieldName = String.format(TDengineConstants.DATE_FORMAT, originField, format);
                    }
                }
            } else {
                fieldName = originField;
            ...
            if (x.getDeType() == 1) {
                ...
                if (x.getDeExtractType() == 0) {
                    if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                        fieldName = String.format(format,
                                String.format(TDengineConstants.DATE_FORMAT, String.format(TDengineConstants.STR_TO_DATE, originField, StringUtils.isNotEmpty(x.getDateFormat()) ? x.getDateFormat() : TDengineConstants.DEFAULT_DATE_FORMAT), "%Y"),
                                String.format(TDengineConstants.QUARTER, String.format(TDengineConstants.STR_TO_DATE, originField, StringUtils.isNotEmpty(x.getDateFormat()) ? x.getDateFormat() : TDengineConstants.DEFAULT_DATE_FORMAT)));
                    } else {
                        fieldName = String.format(TDengineConstants.DATE_FORMAT, String.format(TDengineConstants.STR_TO_DATE, originField, StringUtils.isNotEmpty(x.getDateFormat()) ? x.getDateFormat() : TDengineConstants.DEFAULT_DATE_FORMAT), format);
                    }
                } else {
                    String cast = String.format(TDengineConstants.CAST, originField, TDengineConstants.DEFAULT_INT_FORMAT) + "/1000";
                    String from_unixtime = String.format(TDengineConstants.FROM_UNIXTIME, cast, TDengineConstants.DEFAULT_DATE_FORMAT);
                    if (StringUtils.equalsIgnoreCase(x.getDateStyle(), "y_Q")) {
                        fieldName = String.format(format,
                                String.format(TDengineConstants.DATE_FORMAT, from_unixtime, "%Y"),
                                String.format(TDengineConstants.QUARTER, from_unixtime));
                    } else {
                        fieldName = String.format(TDengineConstants.DATE_FORMAT, from_unixtime, format);
                    }
                }
            } else {
    ...
    private List<SQLObj> getXWheres(ChartViewFieldDTO x, String originField, String fieldAlias) {
        ...
        if (CollectionUtils.isNotEmpty(x.getFilter()) && x.getFilter().size() > 0) {
            x.getFilter().forEach(f -> {
                String whereName = "";
                String whereTerm = transTdengineFilterTerm(f.getTerm());
                String whereValue = "";
                ...
                if (StringUtils.equalsIgnoreCase(f.getTerm(), "null")) {
                    ...
                    whereValue = String.format(TDengineConstants.WHERE_VALUE_VALUE, f.getValue());
                }
                list.add(SQLObj.builder()
                        .whereField(whereName)
    ...
    private SQLObj getYFields(ChartViewFieldDTO y, String originField, String fieldAlias) {
        ...
        if (StringUtils.equalsIgnoreCase(y.getOriginName(), "*")) {
            fieldName = TDengineConstants.AGG_COUNT;
        } else if (SQLConstants.DIMENSION_TYPE.contains(y.getDeType())) {
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "count_distinct")) {
                fieldName = String.format(TDengineConstants.AGG_FIELD, "COUNT", "DISTINCT " + originField);
            } else if (StringUtils.equalsIgnoreCase(y.getSummary(), "group_concat")) {
                fieldName = String.format(TDengineConstants.GROUP_CONCAT, originField);
            } else {
                fieldName = String.format(TDengineConstants.AGG_FIELD, y.getSummary(), originField);
            }
        } else {
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "avg") || StringUtils.containsIgnoreCase(y.getSummary(), "pop")) {
                String cast = String.format(TDengineConstants.CAST, originField, y.getDeType() == 2 ? TDengineConstants.DEFAULT_INT_FORMAT : TDengineConstants.DEFAULT_FLOAT_FORMAT);
                String agg = String.format(TDengineConstants.AGG_FIELD, y.getSummary(), cast);
                fieldName = String.format(TDengineConstants.CAST, agg, TDengineConstants.DEFAULT_FLOAT_FORMAT);
            } else {
                String cast = String.format(TDengineConstants.CAST, originField, y.getDeType() == 2 ? TDengineConstants.DEFAULT_INT_FORMAT : TDengineConstants.DEFAULT_FLOAT_FORMAT);
                if (StringUtils.equalsIgnoreCase(y.getSummary(), "count_distinct")) {
                    fieldName = String.format(TDengineConstants.AGG_FIELD, "COUNT", "DISTINCT " + cast);
                } else {
                    fieldName = String.format(TDengineConstants.AGG_FIELD, y.getSummary(), cast);
                }
            }
        }
        return SQLObj.builder()
    ...
    public String sqlForPreview(String table, Datasource ds) {
        return "SELECT * FROM " + String.format(TDengineConstants.KEYWORD_TABLE, table);
    }
